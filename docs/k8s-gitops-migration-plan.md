...
- **⚠ OUTSTANDING OPERATOR ACTION (UI only):** both GHCR packages are still **private** (anon pull `403`; default private on first publish). **No REST API** exists to flip GHCR visibility and the `gh` token lacks package scopes — the operator must set each package **Public** in the web UI (Package → Settings → Danger Zone → Change visibility). URLs in `docs/versions.md`. **Phase 8 will `ImagePullBackOff` until this is done** (or an imagePullSecret is re-added). Phase 8 can also then upgrade the pins from SHA tag → `@sha256:` digest.
